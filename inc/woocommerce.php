...
add_filter( 'woocommerce_get_price_html', __NAMESPACE__ . '\\show_exc_vat', 2, 10 );

function show_exc_vat($price, $product) {

	
    if(is_singular() && has_term(22, 'product_cat', $product->get_id())){

        if($product->is_type('variable-subscription')){

        $price = 'From '.wc_price(wc_get_price_excluding_tax( $product )).' + VAT / month';

        }
        else{
            $price = wc_price(wc_get_price_excluding_tax( $product )).' + VAT';
        }
    }
    elseif(is_singular() && has_term(22, 'product_cat', $product->get_parent_id())){
        $price = wc_price(wc_get_price_excluding_tax( $product )).' + VAT';
    }
